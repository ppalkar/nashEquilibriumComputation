...
#gamma variables
var gamma1{s in S, a1 in A1[s]};
var gamma2{s in S, a2 in A2[s]};
...
minimize obj:
    (sum{s in S , a1 in A1[s]} (gamma1[s,a1] - (1 - h1[s,a1]))) + 
    (sum{s in S, a2 in A2[s]} (gamma2[s,a2] - (1 - h2[s,a2])));
...
subject to K1_v1_constraint{s in S, a1 in A1[s]}:
    (v1[s] - (sum{a2 in A2[s]} R1[s,a1, a2]*g[s,a2]) 
    - beta*(sum{a2 in A2[s]} T1[s, a1,a2]*g[s,a2])) <= K1*h1[s,a1] ;

subject to K2_v2_constraint{s in S, a2 in A2[s]}:
    (v2[s] - (sum{a1 in A1[s]} R2[s,a1, a2]*f[s,a1]) 
    - beta*(sum{a1 in A1[s]} T2[s, a1,a2]*f[s,a1])) <= K2*h2[s,a2] ;


subject to gamma1_constraint1{s in S, a1 in A1[s]}:
    gamma1[s,a1] >= (1 - h1[s,a1]);

subject to gamma2_constraint1{s in S, a2 in A2[s]}:
    gamma2[s,a2] >= (1 - h2[s,a2]);

subject to gamma1_constraint2{s in S, a1 in A1[s]}:
    gamma1[s,a1] >= f[s,a1];

subject to gamma2_constraint2{s in S, a2 in A2[s]}:
    gamma2[s,a2] >= g[s,a2];
